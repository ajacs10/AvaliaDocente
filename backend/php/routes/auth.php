...
try {
    ...
    $professorIdSelect = isset($userColumns['professor_id']) ? ', professor_id' : ', NULL AS professor_id';
    $stmt = $db->prepare(
    ...
    // Determine professor_id: prefer explicit mapping stored in usuarios.professor_id,
    // otherwise fall back to matching by name (legacy behavior).
    $professorId = null;
    if (($user['tipo'] ?? '') === 'professor') {
        if (!empty($user['professor_id'])) {
            $professorId = (int)$user['professor_id'];
        } else {
            $profStmt = $db->prepare('SELECT id FROM professores WHERE nome = :nome LIMIT 1');
            $profStmt->execute([':nome' => $user['nome']]);
    ...
    json_response([
        'success' => false,
        'message' => 'Nao foi possivel autenticar agora.'
    ], 500);
}
